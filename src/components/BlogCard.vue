...
<template>
  <div
    class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-4 items-center"
  >
    <div v-for="(post, i) in blogPosts" :key="i">
      <div class="card relative">
        <img class="cursor-pointer w-full" :src="post.blogImage" />
        ...
        <div class="ml-4 mt-2">
          <router-link class="link" to="#">ブログタイトル</router-link>
          <div class="flex">
            <span class="block text-gray-500 text-sm py-2 mr-2"
              >編集者：テスト太郎</span
            >
            <span class="block text-gray-500 text-sm py-2"
              >作成者：テスト次郎</span
            >
          </div>
        </div>
